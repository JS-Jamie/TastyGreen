...
const OrderScreen = () => {
  ...
  useEffect(() => {
    ...
    if (!order || successPay || successDeliver) {
      ...
      /*Loop prevention. Without this, it will keep
      refreshing once it's paid */
                  ...
                  {order.orderItems.map((item, index) => (
                    <ListGroup.Item key={index}>
                      <Row>
                        <Col sm={2} md={2}>
                          <Image
                            className='snackImagePlaceOrder'
                            src={item.image}
                            alt={item.name}
                            fluid
                            rounded
        ...
        <Col md={4}>
          <Card>
            <ListGroup variant='flush'>
              ...
              {userInfo && //if they are logged in
                userInfo.isAdmin && // if they are admin
                order.isPaid && // if the order is paid by customer
                !order.isDelivered && (
                  <ListGroup.Item>
                    <Button
                      type='button'
